...
from src.dataset import FewShotDataset, prompt_encoder
...
def main():
    ...
    if "prompt" in model_args.few_shot_type:
        if config.model_type == "roberta":
            ...
            if model_args.few_shot_type == "fprompt":
                model_fn = T5ForFinetuning
            else:
    ...
    train_dataset = FewShotDataset(
        ...
        use_pt=(model_args.few_shot_type == "tprompt"),
    )
    eval_dataset = (
        FewShotDataset(
            data_args,
    ...
    if model_args.few_shot_type == "tprompt":
        prompt_encoder.init_embed(model.t5)
        handle = prompt_encoder.add_embed_hook(model.t5)
        training_args.tprompt = True
    else:
        training_args.tprompt = False
    ...
    if data_args.prompt:
        model.label_words_list = train_dataset.label_words_list
    if output_modes_mapping[data_args.task_name] == "regression":
        # lower / upper bounds
        model.lb, model.ub = bound_mapping[data_args.task_name]
    if data_args.prompt and data_args.task_name == "copa" and "man" in data_args.tag:
        model.copa_train_mapping = train_dataset.label_words_list
        if training_args.do_eval:
            model.copa_eval_mapping = eval_dataset.label_words_list
        if training_args.do_predict:
            model.copa_test_mapping = test_dataset.label_words_list
    if data_args.prompt and data_args.task_name == "record":
        model.record_train_mapping = train_dataset.label_words_list
        if training_args.do_eval:
            model.record_eval_mapping = eval_dataset.label_words_list
        if training_args.do_predict:
            model.record_test_mapping = test_dataset.label_words_list
    if data_args.prompt and data_args.task_name == "wsc":
        model.wsc_train_mapping = train_dataset.label_words_list
        if training_args.do_eval:
            model.wsc_eval_mapping = eval_dataset.label_words_list
        if training_args.do_predict:
            model.wsc_test_mapping = test_dataset.label_words_list
    ...
    def build_compute_metrics_fn(task_name: str) -> Callable[[EvalPrediction], Dict]:
        def compute_metrics_fn(p: EvalPrediction, guids):
            # Note: the eval dataloader is sequential, so the examples are in order.
            # We average the logits over each sample for using demonstrations.
            if task_name == "record":
                return compute_metrics_mapping[task_name](
                    task_name, p.predictions, p.predictions
                )
            ...
            logits = predictions.reshape(
                [
                    test_dataset.num_sample
                    if test_dataset
                    else eval_dataset.num_sample,
                    -1,
                    num_logits,
                ]
            )
            ...
            if task_name == "multirc":
                return compute_metrics_mapping[task_name](
                    task_name, preds, label_ids, guids=guids
                )
            else:
    ...
    if training_args.do_train:
        if (
            data_args.prompt
            and data_args.task_name == "copa"
            and "man" in data_args.tag
        ):
            trainer.model.copa_mode = "train"
        if data_args.prompt and data_args.task_name == "record":
            trainer.model.record_mode = "train"
        if data_args.prompt and data_args.task_name == "wsc":
            trainer.model.wsc_mode = "train"
        trainer.train(
    ...
    if training_args.do_predict:
        ...
        if (
            ...
            trainer.model.copa_mode = "test"
        if data_args.prompt and data_args.task_name == "record":
            trainer.model.record_mode = "test"
        if data_args.prompt and data_args.task_name == "wsc":
            trainer.model.wsc_mode = "test"
