...
case $TASK in
    ...
    CB)
        ...
        ;;
    COPA)
        TEMPLATE=*cls**sent-_0*_or*sent-_1*?*sent-_2*,*+sent_3**mask*.*sep+*
        MAPPING="{0:'False',1:'True'}"
        TASK_EXTRA="--max_seq_len 256"
        ;;
    MultiRC)
        TEMPLATE=*cls**sent_0*_Question:*+sent_1*_Is_it*+sent_2*?*mask*.*sep+*
        MAPPING="{0:'No',1:'Yes'}"
        TASK_EXTRA="--max_seq_len 512"
        ;;
    ReCoRD)
        ...
        ;;
    WiC)
        TEMPLATE=*cls**sent_0**+sent_1*_Does*+sent_2*_have_the_same_meaning_in_both_sentences?*mask**sep+*
        MAPPING="{False:'No',True:'Yes'}"
        TASK_EXTRA="--max_seq_len 256"
        ;;
    WSC)
        TEMPLATE=*cls**sent_0*_The_pronoun*+sent_2*_refers_to*mask*.*sep+*
